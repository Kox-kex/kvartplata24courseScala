...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy(identity).view.mapValues(_.size).toList.sortBy(_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    combHelper1(occurrences)
      .foldLeft(List(List[(Char, Int)]()))(
        (e1, e2) => e1 ::: combHelper2(e1, e2))
  }

  def combHelper1(occurrences: Occurrences): List[Occurrences] = {
    occurrences.map(occ => (1 to occ._2)
      .map(o => (occ._1, o))
      .toList)
  }

  def combHelper2(e1: List[Occurrences], e2: Occurrences): List[Occurrences] = {
    for {
      elem1 <- e1
      elem2 <- e2
...
object Test extends App {
  ...
  Anagrams.combinations(abba).toSet
